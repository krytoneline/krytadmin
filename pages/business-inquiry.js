...
function BusinessInquiry(props) {
    ...
    function date({ value }) {
        return (
            < div>
                <p className='text-custom-black text-base font-normal text-center'>{moment(new Date(value)).format('DD MMM YYYY')}</p>
            </div>
        )
    }
    ...
    return (
        <section className=" w-full h-full bg-transparent md:pt-5 pt-2 pb-5 pl-5 pr-5">
            <p className="text-white font-bold  md:text-[32px] text-2xl md:pb-0 pb-3">{t("Business Inquiry")}</p>

            <section className='px-5 pt-5 md:pb-32 pb-28 bg-white h-full rounded-[12px] overflow-scroll md:mt-9 mt-5'>
                {/* shadow-2xl  */}
                {userRquestList?.length > 0 && <div className='bg-white border border-custom-lightGrayColor w-full md:h-[70px] rounded-[10px] md:py-0 py-5 md:px-0 px-5'>
                    <div className='md:grid md:grid-cols-10 grid-cols-1 w-full h-full '>
                        ...
                        <div className='col-span-8 flex md:flex-row flex-col md:justify-between justify-start md:items-center items-start'>
                            ...
                            <button className="h-[38px] w-[110px] bg-custom-red text-white text-base	font-normal rounded-[8px] md:mr-5 md:mt-0 mt-5" onClick={() => { getOrderBySeller(); setSelctDate('') }}>{t("reset")}</button>
                        </div>
                    </div>
                </div>}
                    ...
                    <div className='w-[310px] relative'>
                        <div className='w-full p-5'>
                            ...
                            {cartData?.productDetail?.map((item, i) => (
                                <div className="w-full" key={i} onClick={() => { router.push(`/orders-details/${cartData?._id}?product_id=${cartData?.productDetail[0]?._id}`) }}>
                                    <div className='grid grid-cols-4 w-full gap-2 py-5'>
                                        ...
                                        <div className='col-span-2 w-full'>
                                            ...
                                            {item?.color && <div className="flex justify-start items-center gap-1">
                ...
                {userRquestList?.length > 0 && <div className=''>
...
export default isAuth(BusinessInquiry)
