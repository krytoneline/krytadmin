  ...
  const info = ({ value, row }) => {
    ...
    return (
      <div className=" p-4  flex items-center  justify-center">
        <button
          className="h-[38px] w-[93px] bg-[#00000020] text-custom-red text-base	font-normal rounded-[8px]"
          onClick={() => {
            setviewPopup(true);
            setPopupData(row.original);
            setdriverdata([
...

                {
                  <div className="flex md:justify-center justify-start items-center min-w-[400px] md:border-l-2 md:border-l-gray-300">
                    <div className="flex flex-col justify-start items-start md:pl-5 md:w-[80%]">
                      ...
                      {popupData?.subscription?.plan?.plantype && (
                        <div className="flex justify-between items-center w-full pt-2">
                          <p className="text-sm font-normal text-custom-black">
                            {t("subscription_name")}
                          </p>
                          <p className="text-sm font-bold text-custom-black ml-2">
                            {popupData?.subscription?.plan?.plantype}
                          </p>
                        </div>
                      )}
                      {popupData?.subscription?.expiry_date && (
                        <div className="flex justify-between items-center w-full pt-2">
                          <p className="text-sm font-normal text-custom-black">
                            {t("expiry_date")}
                          </p>
              ...
              <div className="md:h-12">
                <div className="flex  mt-5  justify-center  gap-5">
                  {popupData?.store?.verification != "Verified" && (
                    <button
                      className="text-white text-lg font-bold w-[274px] h-[50px] rounded-[12px] bg-custom-lightBlue"
                      onClick={() => {
                        if (!popupData?.store) {
                          return props.toaster({
                            type: "error",
                            message: "Seller store has not been created yet.",
                          });
                        }
                  ...
                  {popupData?.store?.verification != "Suspend" && (
                    <button
                      className="text-white text-lg font-bold w-[274px] h-[50px] rounded-[12px] bg-custom-lightRed"
                      onClick={() => {
            ...
            <div className="relative w-[300px] md:w-[360px] h-auto  bg-white rounded-[15px] m-auto">
              ...
              <div className="px-5 py-10">
                <form className="w-full" onSubmit={F}>
                  ...
                  />
                  <button
                    className="bg-custom-gray  md:h-[50px] h-[40px] w-full rounded-[5px] md:text-xl text-base text-white font-normal mt-5"
                    type="submit"
                  >
                    Submit
                  </button>
