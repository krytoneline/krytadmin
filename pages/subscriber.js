  ...
  useEffect(() => {
    if (user?.type === "SELLER") {
      if (
        ...
        user?.subscription?.expiry_date &&
        new Date() < new Date(user?.subscription?.expiry_date)
      ) {
    ...
    Api("put", "updateplaninuser", data, router).then(
      (res) => {
        ...
        if (res.status) {
          ...
          setUser({
            ...user,
            subscription: res.data.newresponse.subscription,
            trial_subscription: res.data.newresponse.trial_subscription,
          });
    ...
    <>
      <div className="w-full h-full bg-transparent md:pt-5 pt-2 pb-5 pl-5 pr-5">
        ...
        <section className="md:px-5 px-3 md:pt-5 pt-3 md:pb-32 pb-28 bg-white h-full rounded-[12px] overflow-scroll md:mt-9 mt-5">
          <p className="text-black md:text-4xl text-2xl font-bold text-center">
            {t("Our Subscription Plans")}
          </p>
          <p className="text-black font-light md:text-base text-base text-center md:pt-3 pt-3">
            {t("Pick an account plan that fits your workflow")}
          </p>
            ...
            {plandata.map((item, i) => (
              <div key={i} className="w-full">
                <div className="border-[3px] border-custom-newBlackColor rounded-[20px] p-5 bg-[#24243010] w-full">
                  <p className="text-black md:text-[25px] text-xl font-medium">
                    {item?.plantype}
                  </p>
                  {/* {t("Pro")} */}

                  <p className="md:text-[50px] text-2xl text-custom-red font-bold md:pt-5 pt-3">
                    {item?.currency}
                    {item?.price}
                    {item.month < 12 && (
                  ...
                  <div className="md:pt-5 pt-3 md:pb-2  md:px-5">
                    {item?.extra_feature.map((plan, idx) => (
                      ...
                      >
                        ...
                        <p className="text-black md:text-[19px] text-base font-semibold">
                          {plan?.key}
                        </p>
                      </div>
                    ))}
                  ...
                  <button
                    className="bg-custom-red w-full h-[39px] rounded-[20px] text-white md:text-lg text-base font-medium"
                    onClick={() => {
                      ...
                      if (item?.plantype === "Trial") {
                        if (!user.trial_subscription) {
                          ...
                          props.toaster({
                            type: "error",
                            message: "You have already used this trial plan",
                          });
                        }
                      } else {
                        // buySubscription()
                          ...
                          createOrder={(data, actions) => {
                            return actions.order.create({
                              ...
                              purchase_units: [
                                {
                                  amount: {
                                    value: String(planDetails?.price || "1.00"),
                                  },
                                  currency_code: "USD",
                                },
                              ],
          ...
          <div className="fixed top-0 left-0 w-screen h-screen bg-black/30 flex justify-center items-center z-50">
            <div className="relative w-[300px] md:w-[360px] h-auto  bg-white rounded-[15px] m-auto">
              ...
              <div className="px-5 py-10">
                <p className="text-base font-bold text-custom-black text-center">
                  Subscription plan successfully saved
                </p>
                ...
                <button
                  className="bg-custom-red h-[45px] w-full rounded-[12px] NunitoSans text-white font-normal text-base mt-5"
                  onClick={() => {
                    router.replace("/");
                    setOpenPopup(false);
                  }}
          ...
          <div className="fixed top-0 left-0 w-screen h-screen bg-black/30 flex justify-center items-center z-50">
            <div className="relative w-[300px] md:w-[600px] h-auto  bg-white rounded-[15px] m-auto">
              ...
              <div className="px-5 py-10 flex flex-col justify-center items-center">
                ...
                <p className="text-xl font-semibold text-black text-center md:pt-5 pt-3">
                  {t("You have not subscribed to the services.")}
                </p>
                <p className="text-sm font-medium text-black text-center md:pt-5 pt-3">
                  {t(
                    "For use our services please subscribe to our services for using the features of our website.",
                  )}
                </p>
                <button
                  className="bg-custom-red h-[37px] w-[99px] rounded-[5px] NunitoSans text-white font-medium text-sm md:mt-5 mt-3"
                  onClick={() => {
                    setSubscribedServicesPopup(false);
                  }}
                >
